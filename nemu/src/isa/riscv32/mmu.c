...
paddr_t isa_mmu_translate(vaddr_t addr, int type, int len) {
    // printf("addr:%x\n", addr);
    // the address is the address in pmem
    uint32_t page_dir_address = (cpu.satp & ETRA_PDA) * PAGE_SIZE;
    uintptr_t level = LEVELS - 1;
    // printf("page_dir_address:%x\n", page_dir_address);

    struct VA va = {};
    va.va_no.val = addr;
    
    // struct PTE *pte_page_table = (struct PTE *)(page_dir_address + va->va_no.fields.VPN_1);
    struct PTE pte_page_table = {};
    
    pte_page_table.PTE_uo.val = paddr_read(page_dir_address + va.va_no.fields.VPN_1 * 4, 4);
    // printf("page_dir_address + va.va_no.fields.VPN_1 * sizeof(PTE):%x\n", page_dir_address + va.va_no.fields.VPN_1 * 4);
    // printf("va.va_no.fields.VPN_1:%d\n", va.va_no.fields.VPN_1);
    // printf("pte_page_table.PTE_uo.val:%x\n", pte_page_table.PTE_uo.val);
    ...
    level -= 1;
    // PTE *page_table_address = (pte_page_table->PTE_uo.union_01.PPN_01) << PT_BASE_ADDR_LR;
    // struct PTE *pte_page_frame = (struct PTE *)(page_table_address + va->va_no.fields.VPN_0);
    uint32_t page_table_address = pte_page_table.PTE_uo.union_01.PPN_01 * PAGE_SIZE;
    // printf("pte_page_table.PTE_uo.union_01.PPN_01:%x\n", pte_page_table.PTE_uo.union_01.PPN_01);
    ...
    // printf("pte_page_frame.PTE_uo.val:%x\n", pte_page_frame.PTE_uo.val);   
